...
void on_pointer_down(int x, int y){
    ...
    else if(ScreenWidth() + PAUSE_BUTTON_X <= x && x < ScreenWidth() + PAUSE_BUTTON_X + (int)pause_button.width
            && MENU_Y <= y && y < MENU_Y + (int)pause_button.height){
        game.toggle_pause();
    }
    else if(ScreenWidth() + NEW_GAME_BUTTON_X <= x && x < ScreenWidth() + NEW_GAME_BUTTON_X + (int)new_game_button.width
            && MENU_Y <= y && y < MENU_Y + (int)new_game_button.height){
        game.new_game();
    }
}
